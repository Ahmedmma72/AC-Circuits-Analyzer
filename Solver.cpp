...
struct Component
{
	...
	complex<double> Y = 0;
	//nodes entered as (-ve) for (node1) and (+ve) for (node2) ,espesially  for VS and CS.
...
int numOfVS = 0, numOfCS = 0, numOfR = 0, numOfC = 0, numOfL = 0, numOfNodes = 0;
...
bool IsOkParamteres(const vector<string>& words, const int& size) {
	if (words.size() == size) return true;
	else return false;
}
...
void FillSourcesData(const vector<string>& words, const int& type) {
	...
	if (type == 0) {
		source.voltage = complex<double>(magntiude * cos(phase * PI / 180), magntiude * sin(phase * PI / 180));
		names.push_back(source.name);
	}
	else if (type == 1) {
		source.current = complex<double>(magntiude * cos(phase * PI / 180), magntiude * sin(phase * PI / 180));
	}
...
bool ReadCircuitData() {
	...
	if (!inputFile.is_open()) {
		...
		while (!inputFile.eof()) {
			...
			else if (words[0].find("H") != string::npos) {//current controlled voltage source
			...
			else if (words[0].find("F") != string::npos) {//current controlled current source
				valid = IsOkParamteres(words, 6);
				if (!valid) {
			...
			else if (words[0].find("E") != string::npos) {//voltage controlled voltage source
				valid = IsOkParamteres(words, 6);
				if (!valid) {
			...
			else if (words[0].find("G") != string::npos) {//voltage controlled current source
				valid = IsOkParamteres(words, 6);
				if (!valid) {
...
void SearchByNodes(const int& node1, const int& node2, vector<Component*>& parrallelComp) {
	for (int i = 0; i < components.size(); i++) {
...
void AddCompInSeries(const int& negNode, const int& posNode, const int& nx, const int& i) {
...
1 - first group of zero voltage sources are for CCVS and they numbered from 1 to numOfH.
2 - second group of zero voltage sources are for CCCS and the numbered from (numOfH + 1) to (numOfH + numOfF).
*/
...
void MatrixB(const int& numOfNodes, const int& numOfVS, const int& numOfH, const int& numOfF, const int& numOfE, const int& numOfZeroVs) {
...
void MatrixD(const int& numOfVS, const int& numOfH, const int& numOfE, const int& numOfZeroVs) {
...
void MatrixA(const int& numOfNodes, const int& numOfVS, const int& numOfH, const int& numOfF, const int& numOfE, const int& numOfZeroVs) {
...
void MatrixE(const int& numOfVS, const int& numOfH, const int& numOfE, const int& numOfZeroVs) {
	int numOfRows = numOfVS + numOfH + numOfZeroVs + numOfE;
...
void MatrixZ(const int& numOfNodes, const int& numOfVS, const int& numOfH, const int& numOfE, const int& numOfZeroVs) {
	int n = numOfNodes + numOfVS + numOfH + numOfZeroVs + numOfE;
...
void MatrixX(const int& numOfNodes, const int& numOfVS, const int& numOfH, const int& numOfE, const int& numOfZeroVs) {
	int n = numOfNodes + numOfVS + numOfH + numOfZeroVs + numOfE;
...
//note that sign isn't important the power required the square
void SetVoltages() {
	...
	for (int i = 0; i < components.size(); i++) {
		...
		if (components[i].name.find("Vs") == string::npos) {
			...
			if (CCVS || VCVS)components[i].voltage = abs(components[i].voltage);
		}
	}
}
...
bool DisplayPower() {
	...
	if (!outputFile.is_open()) {
		...
		outputFile << "[ " << endl;
		for (int i = 0; i < components.size(); i++) {
			outputFile << components[i].name << " complex power is " << components[i].power << endl;
		}
		outputFile << "]" << endl;
		outputFile.close();
		return true;
